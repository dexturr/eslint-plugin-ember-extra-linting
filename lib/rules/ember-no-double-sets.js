  ...
  create: function(context) {
    ...
    return {
      BlockStatement(node) {
        ...
        const multipleSetsForSameArgument = Object.keys(groupedArguments)
                                                  .map(key => groupedArguments[key])
                                                  .filter(argument => argument.length > 1)
                                                  .map(argument => argument[0]);
        // Object.keys does not return symbol properties
        const mutlilpeSetsForThisArg = Object.getOwnPropertySymbols(groupedArguments)
                                              .map(key => groupedArguments[key])
                                              .filter(argument => argument.length > 1)
                                              .map(argument => argument[0]);
                                              
        if (multipleSetsForSameArgument.length > 0 || mutlilpeSetsForThisArg.length > 0) {
          context.report({
            node: node,
