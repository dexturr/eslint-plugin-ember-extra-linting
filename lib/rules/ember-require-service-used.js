...
module.exports = {
  ...
  create: function(context) {
    ...
    const incrementServiceFromArguements = (node) => {
      ...
      CallExpression(node) {
        if (node.callee.name === 'set' || node.callee.name === 'get' || node.callee.name === 'getProperties') {
          if (node.arguments[0].type === 'ThisExpression') {
            incrementServiceFromArguements(node);
          }
        } else if (node.callee && node.callee.object && node.callee.object.type === 'ThisExpression') {
          if (node.callee.property.name === 'get' || node.callee.property.name === 'set' || node.callee.property.name === 'getProperties') {
            incrementServiceFromArguements(node);
          }
          ...
          if (node.arguments[0].type === 'ThisExpression') {
            ...
            if (setObject.type === 'ObjectExpression') {
